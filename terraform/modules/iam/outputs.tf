...
output "role_arn" {
  description = "The IRSA role, or an empty string when create_role is false. This is the value that annotates the Kubernetes service account."
...
output "service_account_annotation" {
  ...
  value       = local.create_role ? "eks.amazonaws.com/role-arn: ${aws_iam_role.app[0].arn}" : "(no role - create_role is false in this stack)"
}
